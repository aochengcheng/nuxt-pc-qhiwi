...
export default {
  ...
  head(ctx) {
    return {
      ...
      title: `【去海外网】北京海外房产,北京海外房产,北京留学_游学，就上去海外网`,
      meta: [
        {
          hid: "keyword",
          name: "keywords",
  ...
  async asyncData(ctx) {
    ...
    let [banner, Hd, foot] = await Promise.all([
      fetch(
        "/dhr/activity/news",
        {
          typeOf: 2,
    ...
    // console.log("ctx.store.state.cityNum.letter::::", ctx.store.state.cityNum);
    ...
    return {
      ...
      HD,
      showCityNum
      // city: ctx.store.state.cityNum.letter
    };
  },
  computed: {
    hot_data() {
